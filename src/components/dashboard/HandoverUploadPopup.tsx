...
import { submitHandover } from "@/lib/actions/handovers";
import FileUploader from "@/components/ui/FileUploader";
import { cn } from "@/lib/utils";

interface HandoverUploadPopupProps {
    contestId: string;
    initialHandover?: any;
    onClose: () => void;
    onSubmit: () => void;
...
export function HandoverUploadPopup({ contestId, initialHandover, onClose, onSubmit }: HandoverUploadPopupProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const result = await submitHandover(contestId, files.map(f => ({ fileUrl: f.fileUrl })));
            ...
            <div className="bg-card border border-border rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden scale-in-center transition-all duration-300">
                ...
                <div className="flex items-center justify-between p-6 border-b border-border bg-muted/30">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-foreground">Handover Files</h2>
                                ...
                                {files.map((file, index) => (
                                    <div
                                        ...
                                        className={cn(
                                            ...
                                            file.isNew ? "border-orange-500/20 bg-orange-500/[0.02]" : "border-border"
                                        )}
                                    >
                                        <div className="flex flex-col gap-4">
                                            <div className="flex items-start justify-between">
                                                <div className={cn(
                                                    "p-3 rounded-xl",
                                                    file.isNew ? "bg-orange-500/10 text-orange-500" : "bg-muted text-muted-foreground"
                                                )}>
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {file.isNew && (
                                                        <span className="text-[8px] font-black uppercase tracking-tighter text-orange-500 flex items-center gap-1">
                ...
                <div className="p-6 border-t border-border bg-muted/30 flex items-center justify-between">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className="px-8 py-2.5 rounded-xl bg-orange-500 text-white hover:bg-orange-600 transition-all font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg shadow-orange-500/20 active:scale-95"
                        >
                            {isSubmitting ? (
                                <>
