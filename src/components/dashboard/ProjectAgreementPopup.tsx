...
import { signProjectAgreementAsClient, signProjectAgreementAsFreelancer } from "@/lib/actions/projectAgreements";

interface ProjectAgreementPopupProps {
    projectId: string;
    bidId?: string;
...
This Agreement is entered into between the Project Owner ("Client") and the Selected Freelancer ("Freelancer") for the delivery of project work.
...
4. PAYMENT
Payment shall be released upon Client's acceptance of the project files. The project budget shall be transferred to the Freelancer's wallet immediately upon acceptance.

5. CONFIDENTIALITY
Both parties agree to keep confidential any proprietary information shared during the project and handover process.
...
export function ProjectAgreementPopup({ projectId, bidId, userType, onClose, onSign }: ProjectAgreementPopupProps) {
    ...
    const handleSign = async () => {
        ...
        try {
            let result;
            if (userType === "client" && bidId) {
                result = await signProjectAgreementAsClient(projectId, bidId);
            } else {
                result = await signProjectAgreementAsFreelancer(projectId);
            }
            ...
            <div className="bg-card border border-border rounded-xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
                ...
                <div className="flex items-center justify-between p-6 border-b border-border">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold">Project Work Agreement</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 border-t border-border bg-card space-y-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                        <input
                            ...
                            className="h-5 w-5 rounded border-border text-purple-500 focus:ring-purple-500"
                        />
                    ...
                    <div className="flex gap-3 justify-end">
                        ...
                        <button
                            ...
                            className="px-6 py-2.5 rounded-lg bg-purple-500 text-white hover:bg-purple-600 transition-colors font-bold disabled:opacity-50 disabled:cursor-not-allowed"
                        >
